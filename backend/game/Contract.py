...
class ContractManager:
    ...
    def generate_contract(self, username: str) -> Contract:
        airports = self.database.get_airports_by_distance(
            "large_airport", 2000, 5000, username, 20
        )
        ...
        print(
            f"{int(time.time())}|'Contract.ContractManager.generate_contract': Generated contract {contract}"
        )
        return contract
