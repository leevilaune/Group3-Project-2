...
class Database:
    ...
    def get_random_airport(self, amount: int) -> list:
        self.cursor.execute(
            f"SELECT * FROM airport WHERE type IN ('large_airport') ORDER BY RAND() LIMIT {amount}"
        )
    ...
    def get_airports_by_distance(
        ...
        :param distance_min: minimum radius that we use to look for new airports
        :param distance_max: maximum radius that we use to look for new airports
        ...
        sql_get_airports_in_distance = f"""
            ...
            HAVING distance <= {distance_max} AND distance > {distance_min}
            ORDER BY distance
            LIMIT {limit};
        """
